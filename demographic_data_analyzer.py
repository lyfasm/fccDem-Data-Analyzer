...
def calculate_demographic_data(print_data=True):
    ...
    num_bachlors = len(df[df["education"] == "Bacherlors"])
    total_num = len(df)
    percentage_bachelors = round(num_bachlors / total_num * 100, 1)
    ...
    higher_education_rich = round(non_percentage_higher / len(lower_education) * 100, 1)
    ...
    rich_percentage = round(len(num_min_workers[num_min_workers.salaries == ">50K"]) / len(num_min_workers) * 100, 1)
